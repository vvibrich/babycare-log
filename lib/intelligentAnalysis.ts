...
export function analyzeRecords(
  ...
  const symptomsThisWeek = recentRecords.filter(
    (r) => r.type === "symptom"
  ).length;
  const medicationsThisWeek = recentRecords.filter(
    (r) => r.type === "medication"
  ).length;
  ...
  const medicationAnalysis = analyzeMedicationFrequency(
    recentRecords,
    childName
  );
  ...
  // 8. Análise de sono e descanso
  const restQualityAnalysis = analyzeRestQuality(records, childName);
  if (restQualityAnalysis) insights.push(restQualityAnalysis);

  // 9. Análise de sintomas intermitentes
  const intermittentSymptomsAnalysis = analyzeIntermittentSymptoms(
    records,
    childName
  );
  if (intermittentSymptomsAnalysis) insights.push(intermittentSymptomsAnalysis);

  // 10. Análise de recuperação instável
  const unstableRecoveryAnalysis = analyzeUnstableRecovery(records, childName);
  if (unstableRecoveryAnalysis) insights.push(unstableRecoveryAnalysis);

  // 11. Análise de múltiplos sintomas
  const multipleSymptomsAnalysis = analyzeMultipleSymptoms(records, childName);
  if (multipleSymptomsAnalysis) insights.push(multipleSymptomsAnalysis);
...
function analyzeFeverPattern(records: Record[]): Insight | null {
  ...
  const consecutiveDays = findConsecutiveDays(
    Array.from(feverDays).map((d) => parseISO(d))
  );
...
function analyzeMedicationFrequency(
  records: Record[],
  childName?: string
): Insight | null {
...
function analyzeRecurrentSymptoms(
  records: Record[],
  childName?: string
): Insight | null {
  const symptoms = records.filter(
    (r) => r.type === "symptom" && r.symptom_type
  );
  ...
  const mostFrequent = Object.entries(symptomCount).sort(
    ([, a], [, b]) => b - a
  )[0];
...
function analyzeTemperature(records: Record[]): Insight | null {
  ...
  const isImproving = recentTemps.every(
    (temp, i) => i === 0 || temp <= recentTemps[i - 1]
  );

  if (
    isImproving &&
    recentTemps[0] < FEVER_THRESHOLD &&
    recentTemps[recentTemps.length - 1] >= FEVER_THRESHOLD
  ) {
    return {
      id: "temperature-improving",
      type: "success",
      title: "Temperatura normalizando",
      description: `A temperatura está diminuindo progressivamente. Última medição: ${recentTemps[0].toFixed(
        1
      )}°C.`,
      icon: "✅",
      priority: 2,
  ...
  const isIncreasing = recentTemps.every(
    (temp, i) => i === 0 || temp >= recentTemps[i - 1]
  );
  ...
  if (isIncreasing && recentTemps[0] >= FEVER_THRESHOLD) {
    return {
      ...
      description: `A temperatura está aumentando. Última medição: ${recentTemps[0].toFixed(
        1
      )}°C. Considere medidas antitérmicas.`,
      icon: "📈",
      priority: 4,
    };
  }
...
function checkSevereSymptoms(
  records: Record[],
  childName?: string
): Insight | null {
  const severeSymptoms = records.filter(
    (r) =>
      r.type === "symptom" &&
  ...
  if (severeSymptoms.length >= 2) {
    const name = childName || "A criança";
    const hasHighFever = severeSymptoms.some(
      (s) => s.temperature && s.temperature >= 39.5
    );
...
function analyzeImprovement(
  ...
  const recentSymptoms = records.filter(
    (r) => r.type === "symptom" && isAfter(parseISO(r.created_at), threeDaysAgo)
  );

  const olderSymptoms = records.filter(
    (r) =>
      r.type === "symptom" &&
      isAfter(parseISO(r.created_at), oneWeekAgo) &&
...
function checkInactivity(records: Record[]): Insight | null {
  ...
  const daysSinceLastRecord = differenceInDays(
    new Date(),
    parseISO(lastRecord.created_at)
  );
...
// 1️⃣ - Sono e descanso insuficiente
function analyzeRestQuality(
  records: Record[],
  childName?: string
): Insight | null {
  const irritability = records.filter(
    (r) => r.symptom_type === "irritacao"
  ).length;
  const appetiteLoss = records.filter(
    (r) => r.symptom_type === "falta_apetite"
  ).length;

  if (irritability + appetiteLoss >= 3) {
    return {
      id: "rest-alert",
      type: "info",
      title: "Possível cansaço ou estresse",
      description: `${
        childName || "A criança"
      } apresentou sinais de irritação e falta de apetite nos últimos dias. Isso pode indicar cansaço ou sono irregular.`,
      icon: "😴",
      priority: 2,
    };
  }
...
// 2️⃣ - Sintomas intermitentes
function analyzeIntermittentSymptoms(
  records: Record[],
  childName?: string
): Insight | null {
  const oneWeekAgo = subWeeks(new Date(), 1);
  const symptoms = records.filter(
    (r) => r.type === "symptom" && isAfter(parseISO(r.created_at), oneWeekAgo)
  );
  const symptomTypes = [...new Set(symptoms.map((s) => s.symptom_type))];

  if (symptomTypes.length >= 2 && symptoms.length >= 5) {
    return {
      id: "symptom-intermittent",
      type: "warning",
      title: "Sintomas intermitentes",
      description: `${
        childName || "A criança"
      } apresentou diferentes sintomas em curto período. Pode indicar um quadro viral oscilante.`,
      icon: "🔄",
      priority: 3,
    };
  }
...
// 3️⃣ - Recuperação instável
function analyzeUnstableRecovery(
  records: Record[],
  childName?: string
): Insight | null {
  const threeDaysAgo = subDays(new Date(), 3);
  const newSymptoms = records.filter(
    (r) => r.type === "symptom" && isAfter(parseISO(r.created_at), threeDaysAgo)
  );
  const improvement = analyzeImprovement(records, childName);

  if (improvement && newSymptoms.length > 0) {
    return {
      id: "unstable-recovery",
      type: "warning",
      title: "Recuperação instável",
      description: `${
        childName || "A criança"
      } apresentou sinais de melhora, mas novos sintomas surgiram em seguida.`,
      icon: "⚖️",
      priority: 4,
    };
  }
...
// 4️⃣ - Muitos sintomas diferentes
function analyzeMultipleSymptoms(
  ...
  if (symptomTypes.length >= 4 && symptoms.length > 6) {
    return {
      id: "multi-symptom",
      type: "alert",
      title: "Quadro com múltiplos sintomas",
      description: `${childName || "A criança"} apresentou ${
        symptomTypes.length
      } tipos de sintomas recentemente. Pode ser necessário acompanhamento médico.`,
      icon: "🚨",
      priority: 5,
    };
  }
